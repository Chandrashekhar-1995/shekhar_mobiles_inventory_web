...
const SupplierDropdown = ({ formData, setFormData, handleChange }) => {
  ...
  const customers = useSelector((store) => store.customers.allCustomers);
  ...
  const filteredByNumber = queryNumber === "" ? suppliers : suppliers?.filter((s) =>
       s.mobileNumber?.includes(queryNumber.trim())
      );

  const filteredByName = queryName === "" ? suppliers : suppliers?.filter((s) =>
       s.name?.toLowerCase().includes(queryName.toLowerCase().trim())
      );
  ...
  const handleSelect = (customer) => {
    setFormData({
      ...
      address: capitalizeWords(customer.address), // Capitalize the address
    });
          ...
          <div className="form-control w-full">
            ...
            <Combobox value={formData.mobileNumber} onChange={handleSelect}>
              <div className="relative">
                ...
                />
                {suppliers && filteredByNumber.length > 0 && (
          ...
          <div className="form-control">
            ...
            <Combobox value={formData.customerName} onChange={handleSelect}>
              <div className="relative">
                ...
                />
                {suppliers && filteredByName.length > 0 && (
                  <Combobox.Options className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg">
